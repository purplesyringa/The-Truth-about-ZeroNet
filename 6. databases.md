...
Object name `admin/data.json` is *regular expression*. Each file that matches this expression is checked against object value. Right now you can see only `to_table` property. That means that each row from `posts` object in any file that matches *regular expression* (`admin/data.json`) is added to `posts` table.
